	...
	function appendTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0, l = treeNodes.length; i < l; i++) {
			...
			html.push("<a id='", node.tId, IDMark_A, "' treeNode", IDMark_A," onclick=\"", (node.click || ''),
				...
				"' title='' treeNode", IDMark_Icon," onfocus='this.blur();' class='", makeNodeIcoClass(setting, node), "' style='", makeNodeIcoStyle(setting, node), "'></button><span pid='",node[setting.treeNodeParentKey],"' mid='",node[setting.treeNodeKey],"' desc='",node[setting.treeNodeDesc],"' id='", node.tId, IDMark_Span,
